        ...
        <section className="flex justify-start items-center flex-wrap gap-4">
                {
                    data.length ? 
                    data.map((task) => (
                        <div key={task.id} onClick={() => window.location.href = `http://localhost:3000/finished/${task.id}`} className="w-60 h-60 flex justify-evenly items-start p-4 flex-col bg-slate-100 rounded-xl hover:bg-slate-50 hover:shadow-lg hover:shadow-blue-200 hover:-translate-y-1 select-none hover:cursor-pointer duration-300">
